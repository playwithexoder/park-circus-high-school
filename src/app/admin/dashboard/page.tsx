...
import {
  ...
  SignOut, Bell, Gear, X, Check, IdentificationCard, Printer, UserCircle
...
export default function AdminDashboard() {
  ...
  const deleteNotice = (id: number) => {
    ...
    showToast("Notice removed from live board.");
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans selection:bg-rose-500 selection:text-white">
      ...
      <header className="bg-slate-950 text-white sticky top-0 z-50 shadow-xl shadow-slate-900/50 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="relative w-10 h-10 shrink-0">
              <svg viewBox="0 0 40 40" className="absolute inset-0 w-full h-full" aria-hidden="true">
                <circle cx="20" cy="20" r="19" fill="black" stroke="rgba(255,255,255,0.15)" strokeWidth="1" />
              </svg>
              <Image src="/assets/logo_png.png" alt="Logo" fill className="object-contain brightness-0 invert p-1 relative z-10" />
            </div>
            <div>
              <p className="font-heading font-bold text-white text-base leading-tight tracking-wide">{schoolInfo.name}</p>
            ...
            <div className="relative">
              ...
              {pendingCount > 0 && (
                <motion.span 
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="absolute -top-1 -right-1 w-4 h-4 bg-rose-500 text-white text-[9px] rounded-full flex items-center justify-center font-bold"
                >
                  {pendingCount}
                </motion.span>
              )}
            </div>
            <div className="w-8 h-8 rounded-full bg-rose-600 flex items-center justify-center text-white text-xs font-bold shadow-md shadow-rose-600/30">
              HM
            </div>
        ...
        {/* Role Badge Strip */}
        <div className="border-t border-white/10 bg-rose-950/40">
          <div className="max-w-7xl mx-auto px-6 py-1.5 flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-rose-300 tracking-wider">HEADMASTER LEVEL ACCESS</span>
        ...
        <div className="bg-slate-900 border-t border-slate-800">
          <div className="max-w-7xl mx-auto px-6 flex items-center gap-6">
            ...
            <button 
              ...
              className={`relative py-3.5 text-sm font-semibold transition-colors flex items-center gap-2 ${
                activeTab === "idcards" ? "text-white" : "text-slate-400 hover:text-slate-200"
              }`}
            >
              <IdentificationCard size={18} />
              <span>Manage ID Cards</span>
              {activeTab === "idcards" && (
                <motion.div 
                  layoutId="activeTabUnderline"
                  className="absolute bottom-0 left-0 right-0 h-0.5 bg-rose-500"
      ...
      <AnimatePresence>
        {toast && (
          <motion.div 
            ...
            className="fixed bottom-6 right-6 z-[9999] bg-slate-900/95 backdrop-blur-md text-white px-5 py-3.5 rounded-2xl shadow-2xl text-sm font-semibold flex items-center gap-3 border border-white/15"
          >
      ...
      <AnimatePresence mode="wait">
        {activeTab === "dashboard" && (
          <motion.main 
            ...
            className="flex-1 px-6 py-8 max-w-7xl mx-auto w-full print:hidden"
          >
            <div className="mb-8">
              <h1 className="font-heading text-2xl font-bold text-slate-900 tracking-tight">Good Morning, Headmaster</h1>
              <p className="text-slate-500 text-sm mt-0.5">Here is an overview of active submissions and institutional tasks requiring attention.</p>
            </div>
            ...
            >
              {[
                ...
                <motion.div 
                  ...
                  className="bg-white rounded-2xl p-5 flex items-center gap-4 shadow-sm border border-slate-200/80 hover:shadow-md transition-shadow"
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">Class V, IX, and XI applicant reviews</p>
                      </div>
                    </div>
                    ...
                    <table className="w-full text-left">
                      <thead>
                        <tr className="border-b border-slate-100">
                          <th className="pb-3 text-xs font-bold text-slate-400 uppercase tracking-wider">App No.</th>
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {apps.map((app) => (
                          <tr key={app.id} className="hover:bg-slate-50/80 transition-colors">
                            <td className="py-3.5 font-mono text-xs text-slate-500 font-medium">{app.id}</td>
                            <td className="py-3.5 font-semibold text-slate-900 text-sm">{app.name}</td>
                            <td className="py-3.5 text-sm text-slate-600">{app.class}</td>
                            <td className="py-3.5">
                              ...
                              {app.status === "Pending" ? (
                                <div className="flex justify-end gap-2">
                                  <motion.button 
                                    ...
                                    className="flex items-center gap-1 text-xs font-bold px-3 py-1.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors shadow-sm shadow-emerald-600/20"
                                  >
                                  ...
                                  <motion.button 
                                    ...
                                    className="flex items-center gap-1 text-xs font-bold px-3 py-1.5 bg-rose-50 text-rose-700 border border-rose-200 rounded-lg hover:bg-rose-100 transition-colors"
                                  >
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">Live school notices visible to public</p>
                    </div>
                  </div>
                      ...
                      {notices.map(n => (
                        ...
                        >
                          ...
                          <motion.button 
                            ...
                            className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-colors ml-3 shrink-0"
                            title="Remove notice"
                          >
              ...
              <div className="space-y-6">
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Publishes to website noticeboard</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all font-medium" 
                      />
                    </div>
                    <div>
                    ...
                    <div className="border-2 border-dashed border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center text-center bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer">
                      ...
                      <p className="text-xs font-medium text-slate-600">Attach Official Circular (Optional)</p>
                    ...
                    <motion.button 
                      ...
                      className={`w-full font-semibold py-3 rounded-xl transition-all shadow-sm text-sm flex items-center justify-center gap-2 ${
                        publishSuccess ? "bg-emerald-500 text-white" : "bg-purple-600 hover:bg-purple-700 text-white shadow-purple-600/25"
                      }`}
                    >
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6">
                  <h3 className="font-heading text-base font-bold text-slate-900 mb-4">Quick Navigation</h3>
                  <div className="space-y-2">
                    {[
                      { label: "View Public Notices Page", href: "/notices" },
                      { label: "School Information & History", href: "/about" },
                      { label: "Faculty Directory", href: "/faculty" },
                      { label: "Dev & Portal Hub", href: "/dev-portal" },
                    ].map((link, i) => (
                      <Link 
                        key={i} 
                        href={link.href} 
                        className="group flex items-center justify-between px-4 py-2.5 rounded-xl bg-slate-50 hover:bg-slate-100/90 transition-all text-sm font-semibold text-slate-700"
                      >
                        <span>{link.label}</span>
                        <span className="text-slate-400 text-xs transition-transform group-hover:translate-x-0.5">→</span>
                      </Link>
                    ))}
                  </div>
                </div>
          ...
          >
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-0.5">Review student records and generate official bulk student identification cards.</p>
              </div>
              ...
              <div className="flex items-center gap-4 bg-white p-2 rounded-xl shadow-sm border border-slate-200">
                <select 
                  ...
                  className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm font-semibold outline-none focus:border-rose-500"
                >
                ...
                <motion.button 
                  ...
                  className="flex items-center gap-2 bg-slate-950 text-white font-semibold px-5 py-2 rounded-lg hover:bg-black disabled:opacity-50 transition-all shadow-sm"
                >
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
              <div className="p-4 border-b border-slate-100 flex items-center gap-4 bg-slate-50/70">
                <label className="text-sm font-bold text-slate-700">Filter by Class:</label>
                <select 
                  value={idFilterClass} 
                  onChange={e => { setIdFilterClass(e.target.value); setSelectedStudents(new Set()); }} 
                  className="bg-white border border-slate-300 rounded-lg px-3 py-1.5 text-sm font-semibold outline-none focus:border-rose-500 min-w-[150px]"
                >
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-slate-200 bg-white">
                      ...
                      <th className="pb-3 pt-3 text-xs font-bold text-slate-400 uppercase tracking-wider">Student</th>
                      <th className="pb-3 pt-3 text-xs font-bold text-slate-400 uppercase tracking-wider">Roll No</th>
                      <th className="pb-3 pt-3 text-xs font-bold text-slate-400 uppercase tracking-wider">Class</th>
                      <th className="pb-3 pt-3 text-xs font-bold text-slate-400 uppercase tracking-wider">DOB</th>
                      <th className="pb-3 pt-3 text-xs font-bold text-slate-400 uppercase tracking-wider text-right px-4">Status</th>
                    </tr>
                  </thead>
                    ...
                    {filteredStudents.map((stu) => (
                      <tr 
                        ...
                        className={`transition-colors cursor-pointer ${
                          selectedStudents.has(stu.id) ? "bg-rose-50/70" : "hover:bg-slate-50/70"
                        }`}
                      ...
                      >
                        ...
                        <td className="py-3.5 font-semibold text-slate-900 text-sm flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center shrink-0 text-slate-500 border border-slate-200">
                            <IdentificationCard size={16} weight="fill" />
                          </div>
                          <span>{stu.name}</span>
                        </td>
                        <td className="py-3.5 text-sm text-slate-600 font-mono">{stu.rollNo}</td>
                        <td className="py-3.5 text-sm text-slate-600">{stu.class}-{stu.section}</td>
                        <td className="py-3.5 text-sm text-slate-600">{stu.dob}</td>
                        <td className="py-3.5 text-right px-4">
                          <span className="inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 uppercase tracking-wide">
                            Ready
                          </span>
                        </td>
                      </tr>
